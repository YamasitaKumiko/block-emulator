	...
	for idx := 0; idx <= len(txlist); idx++ {
		if idx > 0 && (idx%params.InjectSpeed == 0 || idx == len(txlist)) {
			...
			for sid := uint64(0); sid < uint64(params.ShardNum); sid++ {
				...
				rthm.sl.Slog.Printf("send transactions to shard %d...\n", sid)
				send_msg := message.MergeMessage(message.CInject, itByte)
				go networks.TcpDial(send_msg, rthm.IpNodeTable[sid][0])
				rthm.sl.Slog.Printf("send_msg to shard %d...\n", sid)
			}
...
func (rthm *RelayCommitteeModule) TxHandling() {
	rthm.sl.Slog.Printf("read csvFile: %s", rthm.csvPath)
	txfile, err := os.Open(rthm.csvPath)
	if err != nil {
		log.Panic(err)
	}
	defer txfile.Close()
	rthm.sl.Slog.Printf("close csvFile: %s", rthm.csvPath)
	...
	for {
		...
		if len(txlist) == int(rthm.batchDataNum) || rthm.nowDataNum == rthm.dataTotalNum {
			rthm.sl.Slog.Printf("The len of sending txs is: %d", len(txlist))
